...
handleChange = () => {
  ...
  handleCheck = (event) => {
  	...
  	this.state.checkedItems.push(JSON.parse(event.target.value)); 
  ...
  handleExport=()=>{
	...
	XLSX.writeFile(workbook, `hello.xls`);
...
render()
 {
  ...
  const open = Boolean(this.state.ischecked);
    ...
    {this.state.checked?<div className={classes.search}> 
    	...
    	<Button style={{backgroundColor:'#FFDEAD'}} onClick={this.handleExport}>EXPORT EXCEL</Button>
